...
function VerifyEmailForm() {
  ...
  const onSubmit = async (data: FormInput) => {
    ...
    setSuccess(true);
    const result = await signIn('credentials', { email: body.email, password: data.password, redirect: false });
    if (!result?.error) {
      setTimeout(() => {
  ...
  if (!token) {
    return (
      <p className="font-body-md text-body-md text-error">
        This verification link is missing a token. Please sign up again or request a new link.
      </p>
    );
  }
    ...
    return (
      <div className="space-y-stack-md text-center">
        ...
        <p className="font-body-md text-body-md text-on-surface-variant dark:text-surface-variant">
          Your account is ready. Taking you to your dashboard&hellip;
        </p>
      </div>
    );
  }
      ...
      <div>
        ...
        <div className="mt-2 space-y-1">
          ...
          <div className="flex h-1 gap-1">
            {[0, 1, 2, 3].map((i) => (
              <div key={i} className="h-full w-full overflow-hidden rounded-full bg-surface-variant dark:bg-outline/30">
                <div
